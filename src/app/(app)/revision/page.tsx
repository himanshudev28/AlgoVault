...
const RATINGS = [
  ...
  { quality: 4, label: "Good", hint: "Recalled with effort", cls: "border-emerald-500/40 text-emerald-500 hover:bg-emerald-500/10" },
  { quality: 5, label: "Easy", hint: "Instant recall", cls: "border-teal-500/40 text-teal-500 hover:bg-teal-500/10" },
];
...
export default function RevisionPage() {
  ...
  const renderItem = (row: ProgressRow, dueLabel?: boolean) => {
    ...
    return (
      <li key={row.questionId} className="border-t border-zinc-100 first:border-0 dark:border-zinc-800/60">
        <div
        ...
        {isActive && (
          <div className="border-t border-zinc-100 bg-zinc-50/60 px-4 py-4 dark:border-zinc-800/60 dark:bg-zinc-950/40">
            <p className="text-xs text-zinc-500">
              <strong className="text-zinc-700 dark:text-zinc-300">Active recall:</strong> solve it
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {q.link && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="rounded-lg border border-zinc-300 px-3 py-1.5 text-xs font-medium transition-colors hover:border-emerald-400 dark:border-zinc-700"
                >
                  Open on {q.platform} ↗
                </a>
              )}
            ...
            {revealed && (
              <div className="mt-3 space-y-3">
                ...
                {row.solutionCode && (
                  <pre className="overflow-x-auto rounded-xl border border-zinc-200 bg-zinc-950 p-3 font-mono text-xs leading-relaxed text-zinc-200 dark:border-zinc-800">
                    {row.solutionCode}
                  </pre>
                )}
              </div>
  ...
  return (
    <div className="mx-auto max-w-4xl">
      <div className="mb-6">
        ...
        {doneCount > 0 && (
          <p className="mt-2 inline-block rounded-lg bg-emerald-500/10 px-3 py-1.5 text-xs font-medium text-emerald-600 dark:text-emerald-400">
      ...
      <section className={`${card} mb-4 overflow-hidden`}>
        ...
        {due.length === 0 ? (
          <p className="border-t border-zinc-100 px-4 py-8 text-center text-sm text-zinc-500 dark:border-zinc-800/60">
      ...
      {flagged.length > 0 && (
        <section className={`${card} mb-4 overflow-hidden`}>
          <div className="flex items-center gap-2 px-4 py-3">
            ...
            <span className="rounded-full bg-zinc-500/15 px-2 py-0.5 text-xs font-semibold text-zinc-500">
              {flagged.length}
            </span>
          </div>
